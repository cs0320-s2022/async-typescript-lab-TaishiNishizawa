...
const sunVal: HTMLInputElement = document.querySelector("#sun") as HTMLInputElement;
const moonVal: HTMLInputElement = document.querySelector("#moon") as HTMLInputElement;
const risingVal: HTMLInputElement = document.querySelector("#rising") as HTMLInputElement;
const suggestionsLst = document.querySelector("#suggestions") as HTMLInputElement;
...
function updateSuggestions(matches: string[]): void {
  ...
  for (const match of matches) {
    suggestionsLst.innerHTML += '<li tabindex="0">' + match + '</li>';
  }
}
...
document.addEventListener("keyup", (keyEntry : KeyboardEvent) => {
  if (keyEntry.key == "q") {
    updateValues(myMatchData.sun, myMatchData.moon, myMatchData.rising)
    .then(promisRes => postAndUpdate());
  }
});
...
async function updateValues(sunval: string, moonval: string, risingval: string): Promise<void> {
  ...
  sunVal.value = sunval;
  moonVal.value = moonval;
  risingVal.value = risingval;
}
